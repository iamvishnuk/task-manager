      ...
      <div>
        ...
        <div className='mb-3.5 flex items-center justify-between gap-2'>
          <Badge
            ...
            className={cn(
              'inline-flex items-center gap-1.5 rounded-lg border px-2 py-0.75 text-[10px] font-bold tracking-wider uppercase transition-colors',
              user?.role !== 'ADMIN' && 'hover:cursor-pointer',
              task.status === 'TODO' &&
            ...
            title={
              user?.role === 'ADMIN' ? undefined : 'Click to toggle status'
            }
          >
            {task.status === 'TODO' && (
        ...
        {user?.role === 'ADMIN' && (task as any).userEmail && (
      ...
      <div className='mt-5 flex items-center justify-between border-t border-slate-100 pt-3.5 dark:border-slate-800/55'>
        ...
        <div className='flex items-center gap-2'>
          <button
            ...
            className='text-xs font-semibold text-indigo-600 hover:cursor-pointer hover:text-indigo-500 disabled:cursor-not-allowed disabled:opacity-50 dark:text-indigo-400 dark:hover:text-indigo-300'
          >
            {task.status === 'DONE'
              ? 'Mark Incomplete'
              : task.status === 'IN_PROGRESS'
          ...
          <button
            ...
            className='rounded-md p-1.5 text-slate-400 transition-colors hover:cursor-pointer hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-slate-800/60 dark:hover:text-slate-200'
            title={user?.role === 'ADMIN' ? undefined : 'Edit Task'}
          >
          ...
          <button
            ...
            className='rounded-md p-1.5 text-slate-400 transition-colors hover:cursor-pointer hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-red-950/30 dark:hover:text-red-400'
            title={user?.role === 'ADMIN' ? undefined : 'Delete Task'}
          >
